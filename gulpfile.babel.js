...
gulp.task("hbs", function() {
  var dir_arr = fs.readdirSync("./app/template/layout");
  //console.log(dir_arr);

  dir_arr.forEach(i => {
    i.split(".")[1] && new RegExp(i.split(".")[1]).test(i) && reg(i, i.split(".")[0]);
  });
   ...
   */
  function reg(i, name) {
    let dirFile = path.join("./app/template/layout", i);
...
gulp.task("serve", ["scripts", "styles", "hbs"], () => {
  browserSync({
    notify: false,
...
gulp.task("default", ["clean"], cb =>
  runSequence("styles", ["html", "scripts", "images", "copy", "deploy"], "generate-service-worker", cb)
);
